...
class RPYValueWidget(double_value_widget.DoubleValueWidget):
    ...
    def __init__(self, topic_name, attributes, array_index, publisher, rpy_index, parent):
        ...
        if self._rpy_index == 0:
            title = ez_model.make_text(topic_name, attributes + ['roll'], array_index)
        elif self._rpy_index == 1:
            title = ez_model.make_text(topic_name, attributes + ['pitch'], array_index)
        elif self._rpy_index == 2:
            title = ez_model.make_text(topic_name, attributes + ['yaw'], array_index)
        else:
            rospy.logerr('this is impossible, rpy[%d]' % rpy_index)
            title = ez_model.make_text(topic_name, attributes + ['???'], array_index)
